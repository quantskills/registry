...
def verify_publication(root: Path) -> dict:
    ...
    precedence = dataset.get("publication_precedence")
    historical = [row[0] for row in PUBLICATIONS]
    if precedence not in (historical, historical + ["publication.v12.13.on-demand"]):
    ...
    if [latest[key] for key in sorted(latest) if key in expected_ids] != records:
    ...
    expected_publications = set(precedence)
