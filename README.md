...
<p align="center">
  <img alt="public assets" src="https://img.shields.io/badge/public_assets-22_skills-blue">
...
[`scripts/validate_skill.py`](scripts/validate_skill.py) 只做事实判断；语义判断（标签准确性、敏感内容、文档漂移）交给 [`AGENT.md`](AGENT.md) 定义的只读复核 agent，且只写建议、不改仓库。

| 检查 | 级别 | 内容 |
|---|---|---|
| `required-files` | fail | 声明文件（`SKILL.md`/`AGENT.md`）、`README.md`、`LICENSE` 缺一即隔离 |
...
- `skill-*` 仓库用 `SKILL.md` 声明，`agent-*` 仓库用 `AGENT.md` 声明；
...
| 字段 | 约束 |
|---|---|
| `category` | 12 个枚举：skill 类 `trader-research` `factor` `data-api` `replication` `monitor` `analyst` `tooling`；agent 类 `research-agent` `workflow-agent` `review-agent` `data-agent` `automation-agent` |
...
- 配套的只读复核 agent 边界见 [`AGENT.md`](AGENT.md)：只汇总、只建议，疑似泄密只报路径不复述内容。
...
├── schema/
│   ├── frontmatter.schema.json         # SKILL.md / AGENT.md frontmatter 契约
