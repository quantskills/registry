...
截至 2026-07-05：**44 个 skill / 7 个 agent**。分类分布：`工具流程类 / tooling` 12 · `因子库 / factor` 10 · `分析类 / analyst` 9 · `未分类 / uncategorized` 5 · `监控类 / monitor` 4 · `复现类 / replication` 3 · `数据类 / data-api` 2 · `监控 Agent / monitor-agent` 2 · `工作流 Agent / workflow-agent` 2 · `研究 Agent / research-agent` 1 · `风险 Agent / risk-agent` 1；验证级别 L3 ×9 · L2 ×22 · L1 ×15 · documentation ×1 · production ×3 · rules-only ×1。
