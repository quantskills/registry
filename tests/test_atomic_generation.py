...
class AtomicGenerationTests(unittest.TestCase):
    ...
    def test_production_collection_uses_clone_head_not_remote_head(self):
        with tempfile.TemporaryDirectory(dir=ROOT) as tmp:
            ...
            try:
                entries, _ = build_registry.collect_entries(repos, {}, "enforce", inventory={"assets": ["skill-alpha"], "resources": [".github", "join", "quantskills", "registry"]})
            finally:
