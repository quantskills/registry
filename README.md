...
截至 2026-07-16：**63 个 skill / 7 个 agent**。分类分布：`工具流程类 / tooling` 15 · `因子库 / factor` 12 · `分析类 / analyst` 11 · `未分类 / uncategorized` 9 · `监控类 / monitor` 6 · `交易者研究类 / trader-research` 4 · `数据类 / data-api` 3 · `复现类 / replication` 3 · `监控 Agent / monitor-agent` 2 · `工作流 Agent / workflow-agent` 2 · `研究 Agent / research-agent` 1 · `风险 Agent / risk-agent` 1 · `finance-news / finance-news` 1；验证级别 L3 ×9 · L2 ×25 · L1 ×31 · documentation ×1 · production ×3 · rules-only ×1。
