...
class EvaluationProjectionTests(unittest.TestCase):
    ...
    def test_verifier_and_schema_accept_one_additional_asset(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            new_record.update({
                ...
                "source_publication": scores['publication_precedence'][-1],
            })
            ...
            scores.update({"record_count": len(scores['records']), "historical_observation_count": scores['historical_observation_count'] + 1, "catalog_snapshot_id": catalog["snapshot_id"]})
            ...
            history_path = root / 'evaluations' / 'publications' / (new_record['source_publication'] + '.jsonl')
            ...
            manifest['record_count'] = scores['record_count']
            manifest['historical_observation_count'] = scores['historical_observation_count']
            manifest["publications"] = [
                {**row, 'observation_count': row['observation_count'] + (row['publication'] == new_record['source_publication'])}
            ...
            self.assertEqual(result['records'], scores['record_count'])
