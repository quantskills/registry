...
[`scripts/validate_skill.py`](scripts/validate_skill.py) makes factual judgments only; semantic judgments (tag accuracy, sensitive content, documentation drift) belong to the read-only review agent defined in [`AGENT.md`](AGENT.md), which writes recommendations but never modifies repositories.

| Check | Level | What it verifies |
|---|---|---|
| `required-files` | fail | Declaration file (`SKILL.md`/`AGENT.md`), `README.md`, and `LICENSE` must all exist |
...
- `skill-*` repositories declare via `SKILL.md`; `agent-*` repositories via `AGENT.md`;
...
| Field | Constraint |
|---|---|
| `category` | 12 enums — skill side: `trader-research` `factor` `data-api` `replication` `monitor` `analyst` `tooling`; agent side: `research-agent` `workflow-agent` `review-agent` `data-agent` `automation-agent` |
...
As of 2026-06-13: **28 skills / 0 agents**. Categories: `Tooling / tooling` 10 · `Analyst / analyst` 7 · `Factor Library / factor` 3 · `Monitor / monitor` 3 · `Replication / replication` 3 · `Data API / data-api` 2; validation levels L3 ×3 · L2 ×17 · L1 ×8.
...
- the companion read-only review agent's boundaries are in [`AGENT.md`](AGENT.md): summarize and recommend only; suspected secrets are reported by file path without repeating contents.
...
├── AGENT.md                            # Read-only review agent: duties and hard boundaries
...
├── schema/
│   ├── frontmatter.schema.json         # SKILL.md / AGENT.md frontmatter contract
